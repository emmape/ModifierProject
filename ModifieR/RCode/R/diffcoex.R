...
diffcoex <- function(MODifieR_input, value = 20, beta = 6, cor_method = "spearman",
  ...
  dataset1 <- t(MODifieR_input$expression_matrix[,MODifieR_input$group_indici[[1]]])
  dataset2 <- t(MODifieR_input$expression_matrix[,MODifieR_input$group_indici[[2]]])
  ...
  module_list <- lapply(X = modulesC1C2Merged, FUN = construct_modules, annotation_table = annotation_table, settings = settings)
  
  names(module_list) <- names(modulesC1C2Merged)
