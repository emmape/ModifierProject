...
wgcna <- function(MODifieR_input, pval_cutoff = 0.05, min_module_size = 30, deep_split = 2, pam_respects_dendro = F,
  ...
  dataset1 <- list("data" = t(MODifieR_input$expression_matrix[ ,MODifieR_input$group_indici[[1]]]))
  dataset2 <- list("data" = t(MODifieR_input$expression_matrix[ ,MODifieR_input$group_indici[[2]]]))
  ...
  probes_sel <-info[info$ModuleColor %in% rownames(selectiv_consensusCor),]
