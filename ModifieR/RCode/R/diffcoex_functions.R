...
extractModules<-function(colorh1, datExpr, anno, write=F, file_prefix, dir=NULL)
{
  ...
  for (c in unique(colorh1))
  {
    ...
    if(!(c=="grey")){ if (write) {write.table(t(rownames(anno)[which(colorh1==c)]),file=paste(file_prefix,"_","module",".txt",sep=""),sep='\t',quote=F,row.names=paste('DiffCoEx',c,'\t',collapse=NULL),col.names=F,append=TRUE)}}
    i<-i+1
  }
